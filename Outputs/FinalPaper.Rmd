---
title: "Mental Health Care not an Option for Many Worldwide due to Economic and Societal Factors "
author: "Olaedo Okpareke"
thanks: 'Code and data are available at: https://github.com/OlaedoOkpareke/FinalPaperRepository' 
date: "`r format(Sys.time(), '%d %B %Y')`"
abstract: "Mental Health care is as important as physical care, however for many worldwide, it is deemed as unimportant or unattainable. Previous research has found that the choice to seek therapy is heavily related to the income, gender, and personal attitudes towards mental health. We use data from the 2021 Wellcome Trust survey on Mental Health to analyze this through graphs and tables which evaluate the differences in mental health access for different groups, using R [@citeR] and other packages. We also predict the probability that a person belonging to several groups will seek out professional care. We find that the personal income of an individual, relative wealth of the country, gender, and the presence of external support increase the likelihood that a person seeks mental health care. We also find that level of education, and employment status also affect this choice. This all shows the elitism and privilege tied to mental health access, and explaisn why getting adequate care is not an option for many worldwide. In order to increase the acceptance and availability of mental health treatments, it is important to identify the sources of, and lessen, these barriers.  \\par \\textbf {Keywords:} Mental Health, Therapy, Global, Education, Wealth Inequality, Education, Science,Gender"
output:
...
```{r figtwo, out.width="90%", echo=FALSE, fig.cap = "Decision to seek mental health care by Gender, Belief in Mental Science, and Decision to seek Help from friends"}
...
```{r figthree, out.width="90%", echo = FALSE, fig.cap = "Decision to seek mental health care by Relative Wealth of Nation and Educational Status"}
...
```{r figfour, out.width="90%", echo = FALSE, fig.cap = "Decision to Seek mental health care by Employment Status and level of Comfort with personal income"}
...
We do not use residual or pairwise plots as they are not very helpful for a model with a binary response and many binary and categorical explanatory variables. The p value is the probability that we see our test statistic given the the null hypothesis is true. 
...
We check that the predicted values of the response from our final model are binomial which they are as evident from the S shape going from 0 to 1 in (Figure \@ref(fig:figfive)), which shows that all possible values are between 0 to 1, and that the probabilities are not linear. 

```{r figfive, out.width="90%", echo = FALSE, fig.cap = "The Fitted Value Probabilities of the Model"}
