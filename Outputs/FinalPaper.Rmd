...
```{r figone, message=FALSE, fig.height = 3, echo = FALSE, fig.cap = "Distribution of Age by Decision to seek Mental Care"}
...
```{r figtwo, echo=FALSE, fig.cap = "Decision to seek mental health care by Gender, Belief in Mental Science, and Decision to seek Help from friends"}
...
plot6 = ggplot(data = datamod, aes(x = factor(friends), fill = help)) +
  ...
  theme_minimal() +
  coord_flip()+
  labs(fill = "Choice", x = "Sought Help from Friends", y = "Proportion")
...
```{r figthree, echo = FALSE, fig.cap = "Decision to seek mental health care by Relative Wealth of Nation and Educational Status"}
...
```{r figfour, echo = FALSE, fig.cap = "Decision to Seek mental health care by Employment Status and level of Comfort with personal income"}
...
plot2 = ggplot(data = datamod, aes(x = factor(employment), fill = help)) +
  geom_bar(position = "fill") +
  scale_fill_grey() +
  theme_minimal() +
  coord_flip() +
...
We do not use residual or pairwise plots as they are not very helpful for a model with a binary response and many binary and categorical explanatory variables. The p value is the probability that we will see our test statistic given the the null hypothesis is true. 
...
```{r figfive, echo = FALSE, fig.height=4, fig.cap = "The Fitted Value Probabilities"}

model1 = glm(Help ~ factor(friends) + factor(science) + Age + factor(employment) + factor(Gender)+ factor(Education) + factor(Subjective_Income) + factor(wbi),data = datamod, family = "binomial")
...
stepAIC(glm(Help ~factor(friends) + factor(science) + Age + factor(employment) + factor(Gender)+ factor(Education) + factor(Subjective_Income) + factor(wbi) + factor(comfort), family = "binomial",data=datamod), scope=list(lower=lm(Help ~ 1, data=datamod)),direction = "both", k=2)
```
...
model = glm(Help ~ factor(friends) + factor(comfort) + factor(science) + factor(employment) + Age + factor(Gender) + factor(Education) + factor(Subjective_Income) + factor(wbi),data = datamod, family = "binomial")
...
```{r figseven, echo=FALSE, fig.height=3, fig.cap = "General Trend of the Decision to seek mental Help by Age, grouped by Gender" }
# cite this code
...
```{r figeight, echo=FALSE, fig.cap = "Probability of seeking Professional Mental Health Care in Each Country"}
...
datamodtidymodels =
  ...
  set_engine("glm") %>%
  fit(factor(help) ~ factor(friends) + factor(science) + Age + factor(employment) + factor(Gender)+ factor(Education) + factor(Subjective_Income) + factor(wbi),
      data = train)
